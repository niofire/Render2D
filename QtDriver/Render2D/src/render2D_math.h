...
public:
	MathHelper_L();
	~MathHelper_L();
	static float	deg_to_rad(float i_Angle)	{ return (i_Angle/360) * 2 * PI;}
	static float	rad_to_deg(float i_Angle)	{ return (i_Angle * 360 / (2*PI));}
	static Vec3f	cross_product(Vec3f& v1, Vec3f& v2) {return Vec3f(v1.y*v2.z - v1.z*v2.y, v1.z*v2.x - v1.x*v2.z, v1.x*v2.y - v1.y*v2.x);}
	static void		fast_translate(Mat4x4& _m, Vec3f _v);
